...
def load4clips(foldername, framenames, start_index, tuple_len, clip_len, interval):
    #total_labels = [0, 3, 4]
    #label = total_labels[random.randint(0,2)]
...
class UCF101Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        '''
        framefolder = os.path.join('/home3/taoli/exp/vcop/data/frames', videoname[:-4])
...
class UCF101VCPDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return torch.stack(tuple_clip), torch.tensor(label) 
